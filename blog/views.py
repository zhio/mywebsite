...
def get_blog_common_data(request,blogs_all_list):
    paginator = Paginator(blogs_all_list,settings.EACH_PAGE_BLOGS_NUMBER)#每3篇分页
 ...
 #获取博客分类的对应的博客数量
    ...
    context = {}
    context['blogs'] = page_of_blogs.object_list#博客列表
    context['page_of_blogs'] = page_of_blogs#博客页码
    context['page_range']= page_range #博客范围
    context['blog_types'] = Blog_Type.objects.annotate(blog_count=Count('blog'))#博客类型
    context['blog_dates'] = blog_dates_dict#博客日期
...
def blogs_with_type(request,blog_type_pk):
    ...
    context = get_blog_common_data(request,blogs_all_list)
    context['blog_type']=blog_type  #博客类型的名称  
